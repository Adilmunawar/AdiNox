...
const AuthContainer: React.FC<AuthContainerProps> = ({ children }) => {
  return (
    <div className="min-h-screen bg-background text-foreground flex">
      ...
      <div className="hidden lg:flex lg:w-[480px] xl:w-[540px] relative overflow-hidden flex-col justify-between p-10 xl:p-14">
        {/* Background effects */}
        ...
        <div className="relative z-10">
          ...
          >
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                ...
                <p className="text-[9px] text-muted-foreground/40 uppercase tracking-[0.25em]">Security Vault</p>
              </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <p className="text-sm text-muted-foreground/50 mt-4 leading-relaxed max-w-[340px]">
          ...
          <div className="mt-10 space-y-4">
            {features.map((f, i) => (
              ...
              >
                <div className="h-9 w-9 rounded-lg bg-secondary/40 border border-border/20 flex items-center justify-center shrink-0 group-hover:border-primary/20 group-hover:bg-primary/[0.06] transition-colors duration-300">
                  <f.icon className="h-4 w-4 text-muted-foreground/60 group-hover:text-primary/70 transition-colors duration-300" />
                </div>
                <div>
                  <p className="text-[13px] font-semibold text-foreground/90">{f.title}</p>
                  <p className="text-[11px] text-muted-foreground/40 mt-0.5">{f.desc}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        >
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[10px] text-muted-foreground/40">End-to-End Encrypted</span>
            </div>
            <div className="flex items-center gap-1.5">
              <Globe className="h-3 w-3 text-blue-500/70" />
              <span className="text-[10px] text-muted-foreground/40">SOC 2 Compliant</span>
            </div>
          </div>
          <p className="text-[10px] text-muted-foreground/25 mt-4">
            &copy; {new Date().getFullYear()} AdiNox — Developed by <span className="text-primary/50">Adil Munawar</span>
          </p>
        </motion.div>
      </div>

      {/* Divider line */}
      <div className="hidden lg:block w-px bg-gradient-to-b from-transparent via-border/40 to-transparent" />

      {/* Right auth form panel */}
      <div className="flex-1 flex flex-col relative overflow-hidden">
        {/* Ambient orbs for mobile */}
        <div className="lg:hidden absolute inset-0 overflow-hidden pointer-events-none">
          <motion.div
            className="absolute -top-[20%] -right-[10%] w-[500px] h-[500px] rounded-full blur-[160px]"
            style={{ background: 'radial-gradient(circle, hsl(var(--primary) / 0.07), transparent 70%)' }}
        ...
        {/* Mobile footer */}
        <footer className="lg:hidden relative z-10 py-4 border-t border-border/20">
